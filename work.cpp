   ...
   关于java：
    ...
    握紧天，二分查找刷了一个题，图的dfs刷了一个，滑动窗口的异位，以及dp的硬币；
     ...
     C++中自定义sort
      ...
      华为笔试当时的第二题，是一个并查集，我希望你晚上把这个能够写出来
      我现在在华硕进行操作并提交
      4.22
      图的整体算法。
      kurusiar与并查集是一样的。
      ...
      4.23
      早上看了prime算法，这个算法的本质是用bfs做的延伸。bfs算法如何 查看层数？while里面套一层for
      这里是用了prority_queque<>---delaytype?
      prime的方法，是优先选择最小的那个节点
      迪杰斯特拉的方法，在这个基础上，while里面，没有用visit，采用的方案是不断更新，最后出战的方法。
      之后看栈，栈的题，恶心的就是跟string结合，尝试了阿里灵犀哪一个题。还是不好做，find_first_of
